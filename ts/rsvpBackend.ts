...
export async function submitRsvp(data: RsvpSubmission): Promise<void> {
  ...
  if (firestoreResult.status === "rejected" && sheetsResult.status === "rejected") {
    throw new Error(
      `Both RSVP backends failed. Firestore: ${firestoreResult.reason}. Google Sheets: ${sheetsResult.reason}.`
    );
  }
